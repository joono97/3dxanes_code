...
def rotcen(fn, sli=None, cen_range=(450, 550, 1)): #회전중심 설정해야함 샘플마다 조금씩 구간다르게 해야함.
    ...
    norm = rmv_stripe(norm, stripe_type="fw_stripe")
    print(f"Denoising...")
...
def recon(fn, rot_cen, nchunk, sli, col=[], stripe_type='fw_stripe', denoise_type="gaussian"):
    time_s = time.time()
...
if __name__ == '__main__':
    ...
    for i in range(0,len(flist)):
        rec =   rotcen(flist[i], sli=150)
        #rot_cen = np.loadtxt(tomo_dir + '/rotcen.txt')
        #data_recon = recon(flist[i], rot_cen=rot_cen[i], nchunk=500, sli=[450, 700])
